...
class ColdDTA(nn.Module):
    def __init__(self, device, PRECISION=2, embed_dim=128):
        super().__init__()
        filter_num = 32
        out_dim = 1
        ...
        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim)).to(self.device)
    ...
    def infer(self, data):
        ...
        x = self.fc2(x)
        x = self.batchnorm2(x)
        fusion_features = self.relu(x)

        x = self.fc25(fusion_features)
        x = self.batchnorm3(x)
        fusion_num_features = self.relu(x)

        x = self.fc3(fusion_features)
        ...
        fusion_num_features = fusion_num_features / fusion_num_features.norm(dim=1, keepdim=True)
        ...
        num_logits_per_dc = logit_scale * fusion_num_features @ number_features.t()
        # logits_per_dc =  fusion_features @ text_features.t()
